...
/**
 * Users with VMAX card
 * @author Oussama
 *
 */
public class VmaxUser extends User implements Pricing, Return_Rent, Statistic, Observable{

	
	/**
	 * @param myVelib velib network from this user
	 */
	public VmaxUser(MyVelib myVelib) {
		super(myVelib);
	}
	...
	public int price(int minutes) throws BikeTypeInvalid{
		if(this.getBike().getType().equalsIgnoreCase("ELECTRIC") || this.getBike().getType().equalsIgnoreCase("MECHANIC") ) {
	...
	/**
	 * return of a bike at a given time and at a given station
